...
import { Search, Clock, MapPin, Video, Globe, X, CheckCircle, Play, Calendar, DollarSign } from "lucide-react"
...
function AcademyContent() {
  ...
  function switchTab(tab: Tab) {
    setActiveTab(tab)
    router.replace(`/academy-b?tab=${tab}`, { scroll: false })
  }
          ...
          {activeTab === "sessions" && (
            <div className="flex gap-10 items-start">
              ...
              <div className="w-72 shrink-0">
                <h3 className="text-base font-semibold text-[#1e1b4b]">Sesiones anteriores</h3>
                <p className="mt-0.5 text-xs text-gray-400">Sesiones grabadas disponibles cuando quieras.</p>
                <div className="mt-4 flex flex-col gap-4">
                  {pastVideos.map(v => (
                    <div key={v.id} className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
                      <div className="flex h-36 items-center justify-center bg-[#eef0fe]">
                        <div className="flex h-10 w-10 items-center justify-center rounded-full bg-[#4f46e5]">
                          <Play className="h-4 w-4 text-white" fill="white" />
                        </div>
                      </div>
                      <div className="p-3">
                        <p className="text-xs font-semibold text-[#1e1b4b] leading-snug line-clamp-2">{v.title}</p>
                        ...
                        <button className="mt-3 flex w-full items-center justify-center gap-1.5 rounded-lg border border-[#4f46e5] py-1.5 text-xs font-semibold text-[#4f46e5] hover:bg-[#eef0fe] transition-colors">
...
export default function AcademyBPage() {
  return (
    <Suspense>
      <AcademyContent />
    </Suspense>
